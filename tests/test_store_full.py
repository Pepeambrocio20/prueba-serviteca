...
class TestStoreFull(unittest.TestCase):
    ...
    def test_disminuir_existente_ok_y_actualiza_umbral(self):
        antes = self.store.get_inventario_por_llanta(self.ll1.id).cantidad_disponible
    ...
    def test_venta_un_item(self):
        ...
        self.assertEqual(self.store.get_inventario_por_llanta(self.ll1.id).cantidad_disponible, 13)
    ...
    def test_venta_multiple_items(self):
        ...
        self.assertEqual(self.store.get_inventario_por_llanta(self.ll1.id).cantidad_disponible, 13)
        self.assertEqual(self.store.get_inventario_por_llanta(self.ll2.id).cantidad_disponible, 2)

    def test_bloqueo_sin_stock_no_altera(self):
        inv1 = self.store.get_inventario_por_llanta(self.ll1.id).cantidad_disponible
        inv2 = self.store.get_inventario_por_llanta(self.ll2.id).cantidad_disponible
        with self.assertRaises(StockInsuficiente):
            self.store.registrar_venta(self.cl.id, self.asr.id, [(self.ll1.id, 2), (self.ll2.id, 999)])
        self.assertEqual(self.store.get_inventario_por_llanta(self.ll1.id).cantidad_disponible, inv1)
        self.assertEqual(self.store.get_inventario_por_llanta(self.ll2.id).cantidad_disponible, inv2)
        self.assertEqual(len(self.store.ventas.list()), 0)

    def test_venta_stock_justo(self):
        inv = self.store.get_inventario_por_llanta(self.ll2.id)
        ...
        self.assertEqual(self.store.get_inventario_por_llanta(self.ll2.id).cantidad_disponible, 0)
